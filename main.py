...
class ProfessionalEEMonitorApp(MDApp):
    ...
    def build(self):
        ...
        try:
            ...
            pass  # Running on desktop, or android module missing
        ...
        self.area_input = MDTextField(
            hint_text="Apartment Area (m2)",
            input_filter="int",
            text="80",
        )
        self.temp_out_input = MDTextField(
            hint_text="Outside Temp (C)",
            input_filter="float",
            text="35",
        )
        self.temp_in_input = MDTextField(
            hint_text="Inside Temp (C)",
            input_filter="float",
            text="24",
        )
        self.power_input = MDTextField(
            hint_text="HVAC Power (kWh)",
            input_filter="float",
            text="1.2",
        )

        input_card.add_widget(MDLabel(text="Scan Parameters", font_style="Title", role="medium"))
        ...
        # ---- Scan button (KivyMD 2.0 API) ----
        self.btn_scan = MDButton(
            MDButtonText(
                text="CALCULATE EE SCORE",
                font_style="Title",
                role="large",
            ),
            style="elevated",
    ...
    def calculate_ee(self, instance):
        ...
        if result_data["source"] == "Offline":
            ...
            if result_data["suggested_price"] is not None:
                self.lbl_price.text = (
                    f"Suggested Price: ${result_data['suggested_price']:,.2f}"
                )
